let __games = [
  ...
  window.addEventListener("chemicalLoaded", function(e) {
    const gData = __games
     gData.forEach(async ga => {
      const gicons = document.getElementById('g-icons')

      const g = document.createElement('div')
      g.classList.add('g-icon')
      // Ok so i know rammerhead is very inscure, which is why app and game functions will be split
      // Apps will use UV 3.0 so its more secure 
      ...
      gimage.src = ga.img
      gimage.style.height = "15vh"
      gimage.style.width = "15vh"
      gimage.style.borderRadius = "1rem"

      gimg.style.background = "transparent"
      gimg.style.border = "none"

      const gname = document.createElement('p')
      gname.innerText = ga.name;
      gname.style.textAlign = "center"
      gname.style.fontFamily = "Helvetica, Arial, sans-serif"
      gimg.appendChild(gimage);
      g.appendChild(gimg);
      g.appendChild(gname);
      gicons.appendChild(g);
  })
});
